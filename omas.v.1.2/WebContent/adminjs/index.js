...
$(function(){	
	...
	$('#loginOut').bind('click',function(){
		$.ajax({
			url: 'loginOut',
			success: function(){
				location.href="http://localhost:8080/omas.v.1.2/login";
			}
		});
	});
...
//    $.post('updatePassword?newPass=' + $newpass.val(), function(rtn) {
...
//    })
    $.ajax({
		url: 'updatePassword?newPass=' + $newpass.val(),
		
		dataType: 'json',
		type: 'post',
		success:function(rtn){
			$.messager.alert('系统提示','恭喜，密码修改成功！<br>您的新密码为：' +rtn.message, 'info',function(){
				if(rtn.success){
					$('#w').dialog('close');
					//清空内容
					 $('#txtOldPass').val('');
